...
def interactive_query_creator():
    ...
    if query_type == "1":
        ...
        print("❌ Combined queries not yet implemented")
        return
    elif query_type == "3":
        print("❌ REST API queries not yet implemented")
        return
    else:
...
def create_simple_interactive():
    ...
    required_params = (
        input("Required parameters (comma-separated): ").strip().split(",")
    )
    ...
    print(
        f"2. Test with: python -c \"from queries import get_query; print(get_query('{tool_name}'))\""
    )
