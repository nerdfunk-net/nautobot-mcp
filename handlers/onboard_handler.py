    ...
    async def handle(self, arguments: dict) -> List[TextContent]:
        ...
        if not ip_address:
            return [
                TextContent(
                    type="text",
                    text="Error: ip_address is required for device onboarding",
                )
            ]
        ...
        if not location_name:
            return [
                TextContent(
                    type="text",
                    text="Error: location is required for device onboarding",
                )
            ]
        ...
        if not secret_groups_name:
            return [
                TextContent(
                    type="text",
                    text="Error: secret_groups is required for device authentication",
                )
            ]
        ...
        try:
            ...
            resolution_tasks = [
                ...
                (
                    "secrets_group",
                    self.secrets_group_resolver.resolve(secret_groups_name),
                ),
            ...
            if errors:
                ...
                error_msg += "\n\n**Troubleshooting:**\n"
                error_msg += (
                    "- Use `query_locations_dynamic` to see available locations\n"
                )
                error_msg += (
                    "- Use `query_namespaces_dynamic` to see available namespaces\n"
                )
                ...
                error_msg += (
                    "- Check that secret groups exist in your Nautobot instance\n"
                )
            ...
            device_data = {
                ...
                "update_devices_without_primary_ip": arguments.get(
            ...
            if response.get("job_id"):
                ...
                success_msg += (
                    "The device onboarding job is now running in the background. "
                )
                success_msg += (
                    "You can monitor the job progress in Nautobot's Jobs interface."
                )

                return [TextContent(type="text", text=success_msg)]
            else:
                error_msg = (
                    f"❌ Device onboarding failed: No job ID returned from Nautobot\n"
                )
