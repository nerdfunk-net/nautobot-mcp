...
def parse_role_prompt(prompt: str) -> Dict[str, Any]:
    ...
    if ("show all roles" in prompt_lower or 
        "list all roles" in prompt_lower or 
        "get all roles" in prompt_lower or
        prompt_lower in ["roles", "all roles"]):
    ...
    patterns = [
        ...
        (r'roles?\s+(?:for|of)\s+content\s+type\s+(["\']?)([^"\']+)\1', 'content_types'),
