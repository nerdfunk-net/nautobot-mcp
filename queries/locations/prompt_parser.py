...
def parse_location_prompt(prompt: str) -> Dict[str, Any]:
    ...
    if (
        "show all locations" in prompt
        or "list all locations" in prompt
        or "get all locations" in prompt
        or prompt in ["all locations", "locations"]
    ):
    ...
    match = re.search(
        r"locations?\s+(?:with|having|where)\s+(\w+)\s+([a-zA-Z0-9-_.]+)", prompt
    )
    if match:
        field = match.group(1)
        value = match.group(2)
