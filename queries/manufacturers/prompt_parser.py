...
class ManufacturerPromptParser:
    ...
    def _extract_main_filter(
        ...
        field_lookup_match = re.search(
            r"(?:manufacturers?|vendors?)\\s+(?:with|having)\\s+(\\w+)\\s+((?:not\\s+)?(?:equal|contains|includes|starts\\s+with|begins\\s+with|ends\\s+with|exact|regex|regexp|regular\\s+expression)(?:\\s+to)?)\\s+(.+)",
            prompt,
        )
        ...
        field_match = re.search(
            r"(?:manufacturers?|vendors?)\\s+(?:with|by|having)\\s+(\\w+)\\s+(\\w+)",
            prompt,
        )
        if field_match:
        ...
        show_match = re.search(
            r"show\\s+(?:manufacturer\\s+|vendor\\s+)?(\\w+)\\s*(?:\\s+(\\w+))?", prompt
        )
        if show_match:
