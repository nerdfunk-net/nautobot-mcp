...
class IPAMPromptParser:
    ...
    def _extract_main_filter(self, prompt: str) -> Tuple[Optional[str], Optional[List[str]]]:
        ...
        if ("show all ip addresses" in prompt or 
            "list all ip addresses" in prompt or 
            "get all ip addresses" in prompt or
            prompt in ["all ip addresses", "ip addresses"]):
            return 'show_all', ['true']  # Special marker for show all
        
        # Pattern: "show ip address <address>" - matches IP addresses
        ip_match = re.search(r'(?:show|get|find)\s+(?:ip\s+address|address)\s+(\d+\.\d+\.\d+\.\d+)', prompt)
        if ip_match:
            return 'address', [ip_match.group(1)]
        
        # Pattern: "ip addresses with <field> <operator> <value>" - Enhanced for lookup expressions
        field_lookup_match = re.search(r'ip\s+addresses?\s+(?:with|having)\s+(\w+)\s+((?:not\s+)?(?:equal|contains|includes|starts\s+with|begins\s+with|ends\s+with|exact|regex|regexp|regular\s+expression)(?:\s+to)?)\s+(.+)', prompt)
        ...
        field_match = re.search(r'ip\s+addresses?\s+(?:with|by|having)\s+(\w+)\s+(\w+)', prompt)
        if field_match:
    ...
    def _determine_enabled_fields(self, prompt: str, parsed_result: Dict[str, Any]) -> Dict[str, bool]:
        ...
        if 'all' in prompt and ('properties' in prompt or 'details' in prompt):
            comprehensive_fields = [
                'get_address', 'get_dns_name', 'get_type', 'get_status',
                'get_description', 'get_parent', 'get_tags'
            ]
