...
def parse_status_prompt(prompt: str) -> Dict[str, Any]:
    ...
    if (
        "show all statuses" in prompt_lower
        or "list all statuses" in prompt_lower
        or "get all statuses" in prompt_lower
        or prompt_lower in ["statuses", "all statuses"]
    ):
    ...
    patterns = [
        ...
        (
            r'status(?:es)?\s+(?:with\s+)?description\s+(["\']?)([^"\']+)\1',
            "description",
        ),
