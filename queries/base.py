...
class BaseQuery(ABC):
    ...
    def _execute_graphql(self, client, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.match_type == MatchType.COMBINED:
            ...
            query = self.queries if isinstance(self.queries, str) else self.queries["default"].query
        
        return client.graphql_query(query, arguments)
    
    def _execute_rest(self, client, arguments: Dict[str, Any]) -> Dict[str, Any]:
        """Execute REST query"""
        endpoint = self.queries if isinstance(self.queries, str) else self.queries["default"].query
...
class SimpleGraphQLQuery(BaseQuery):
    ...
    def __init__(self, tool_name: str, description: str, query: str, 
