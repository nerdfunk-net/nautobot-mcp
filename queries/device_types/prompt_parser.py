...
class DeviceTypePromptParser:
    ...
    def _extract_main_filter(self, prompt: str) -> Tuple[Optional[str], Optional[List[str]]]:
        ...
        if ("show all device types" in prompt or 
            "list all device types" in prompt or 
            "get all device types" in prompt or
            prompt in ["all device types", "device types"]):
        ...
        field_lookup_match = re.search(r'device\s+types?\s+(?:with|having)\s+(\w+)\s+((?:not\s+)?(?:equal|contains|includes|starts\s+with|begins\s+with|ends\s+with|exact|regex|regexp|regular\s+expression)(?:\s+to)?)\s+(.+)', prompt)
        ...
        field_match = re.search(r'device\s+types?\s+(?:with|by|having)\s+(\w+)\s+(\w+)', prompt)
        if field_match:
