...
class PrefixPromptParser:
    ...
    def _extract_main_filter(
        ...
        if (
            "show all prefixes" in prompt
            or "list all prefixes" in prompt
            or "get all prefixes" in prompt
            or prompt in ["all prefixes", "prefixes"]
        ):
            return "show_all", ["true"]  # Special marker for show all

        # Pattern: "show prefix <cidr>" - matches CIDR notation like 192.168.1.0/24
        prefix_cidr_match = re.search(
            r"(?:show|get|find)\s+prefix\s+(\d+\.\d+\.\d+\.\d+/\d+)", prompt
        )
        ...
        prefix_length_match = re.search(
            r"(?:with|having)\s+prefix_length\s+(\d+)", prompt
        )
        ...
        within_match = re.search(
            r"(?:prefixes?\s+)?within\s+(\d+\.\d+\.\d+\.\d+/\d+)", prompt
        )
        if within_match:
            return "within", [within_match.group(1)]

        # Pattern: "prefixes within_include <network>"
        within_include_match = re.search(
            r"(?:prefixes?\s+)?within_include\s+(\d+\.\d+\.\d+\.\d+/\d+)", prompt
        )
        ...
        field_match = re.search(
            r"prefixes?\s+(?:with|in|at|by)\s+(\w+)\s+(\w+)", prompt
        )
        if field_match:
